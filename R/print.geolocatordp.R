...
print.geolocatordp <- function(x, ...) {
  ...
  cli::cli_bullets(c("*" = "{.field homepage}: {.url {x$homepage}}"))
  cli::cli_bullets(c("*" = "{.field image}: {.url {x$image}}"))
  cli::cli_bullets(c("*" = "{.field version}: {.val {x$version}}"))
  cli::cli_bullets(c("*" = "{.field created}: {.val {x$created}}"))
  created_datetime <- as.POSIXct(x$created, format = "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
  cli::cli_bullets(c("*" = "{.field created}: {.val {created_datetime}}"))
  embargo_date <- as.POSIXct(x$embargo, format = "%Y-%m-%dT", tz = "UTC")
  ...
  cli::cli_bullets(c("*" = "{.field temporal}: {.val {x$temporal$start}} to {.val {x$temporal$end}}"))
