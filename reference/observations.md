# Get or set observations

`observations()` gets the observations from a GeoLocator Data Package
object.

`observations<-()` is the assignment equivalent. It should only be used
within other functions, where the expected data structure can be
guaranteed.

## Usage

``` r
observations(x)

observations(x) <- value
```

## Arguments

- x:

  A GeoLocator Data Package object, as returned by
  [`read_gldp()`](https://raphaelnussbaumer.com/GeoLocatoR/reference/read_gldp.md).

- value:

  A data frame to assign as observations. Must conform to the
  observations schema specification.

## Value

[`tibble::tibble()`](https://tibble.tidyverse.org/reference/tibble.html)
data frame with observations
